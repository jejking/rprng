...
/**
  ...
  */
case class DrawResult(numbers: Set[Int], superNumber: Int) {

  require(numbers.size == 6, s"Numbers set must be of size 6. It was of size ${numbers.size}.")
  numbers.foreach((i:Int) => require(i >= 1 && i <= 49, "Numbers set may only contain numbers between 1 and 49 inclusive."))
